...
const tabs: { label: string; value: "all" | VendorStatus }[] = [
  { label: "All", value: "all" },
...
export default function AdminVendorsPage() {
  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      const data = snap.docs.map((d) => ({ uid: d.id, ...d.data() } as Vendor));
      setVendors(data);
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-white/30 text-sm mt-0.5">
          ...
          {counts.pending > 0 && (
            <span className="ml-2 text-yellow-400">{counts.pending} pending review</span>
          )}
        </p>
      </div>
