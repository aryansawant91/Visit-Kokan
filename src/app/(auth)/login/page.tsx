    ...
    <div className="flex gap-2 justify-center">
      {digits.map((d, i) => (
        <input
          ...
          onPaste={handlePaste}
          className={`w-11 text-center text-xl font-bold rounded-xl border-2 bg-white
            text-kokan-earth focus:outline-none transition-all ${
            d
...
function Countdown({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setLeft(p => {
        if (p <= 1) { clearInterval(id); onEnd(); return 0 }
        return p - 1
      })
    }, 1000)
...
function saveRLState(s: RLState) { localStorage.setItem(RL_KEY, JSON.stringify(s)) }
...
function incrementAttempts() {
  const s = getRLState()
  saveRLState({ ...s, attempts: s.attempts + 1 })
}
...
export default function LoginPage() {
  ...
  // Capture redirect param on mount — before any navigation happens
  const redirectParam = useRef<string | null>(null)
  useEffect(() => {
    const params   = new URLSearchParams(window.location.search)
    const redirect = params.get("redirect")
    if (redirect && redirect.startsWith("/") && !redirect.startsWith("//")) {
      redirectParam.current = redirect
    }
  }, [])
  ...
  function storeRedirect() {
    ...
    const redirect = params.get("redirect")
    if (redirect && redirect.startsWith("/") && !redirect.startsWith("//")) {
      sessionStorage.setItem("vk_pending_redirect", redirect)
    }
  }
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    setError("")
    setLoading(true)
    // Store redirect in sessionStorage BEFORE any async call
    // so AuthContext can read it even if window.location changes
    storeRedirect()
    try {
      await signInWithEmail(email, password)
    } catch (err) {
      sessionStorage.removeItem("vk_pending_redirect")
      ...
      if (
        ...
        setError("Too many attempts. Try again later or use OTP login.")
      } else {
  ...
  const handleSendOTP = async (e?: React.FormEvent) => {
    ...
    if (!rl.allowed) {
      const m = Math.floor(rl.waitSeconds / 60), s = rl.waitSeconds % 60
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Store redirect — prefer the ref captured on mount, fallback to current URL
      const currentParams  = new URLSearchParams(window.location.search)
      const currentRedirect = currentParams.get("redirect")
      const redirectTarget  =
        redirectParam.current ??
        (currentRedirect &&
      ...
      if (redirectTarget) {
        sessionStorage.setItem("vk_pending_redirect", redirectTarget)
      }

      const { signInWithCustomToken } = await import("firebase/auth")
      const { auth: firebaseAuth }    = await import("@/lib/firebase")
      await signInWithCustomToken(firebaseAuth, tokenData.token)

      // onAuthStateChanged in AuthContext handles redirect from here
  ...
  const handleGoogle = async () => {
    ...
    // Store redirect BEFORE popup opens
    storeRedirect()
    try {
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-2 mt-3">
              <div className="w-7 h-7 rounded-full bg-kokan-sand flex items-center justify-center text-xs font-bold text-kokan-earth">
                P
              </div>
              <div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="flex bg-kokan-sand/20 rounded-xl p-1 mb-6 gap-1">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg
                ...
                tab === "password"
                  ? "bg-white text-kokan-green shadow-sm"
                  : "text-kokan-earth/50 hover:text-kokan-earth"
              }`}
            >
          ...
          {tab === "password" && (
            <form onSubmit={handlePasswordLogin} className="space-y-4" noValidate>
              <div>
                <label className="block text-xs font-semibold text-kokan-earth/60 mb-1.5 uppercase tracking-wide">
                  Email
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full border border-kokan-sand rounded-xl pl-10 pr-4 py-3 text-sm
                      text-kokan-earth bg-white focus:outline-none focus:ring-2
                      focus:ring-kokan-green/30 placeholder:text-kokan-earth/30"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-semibold text-kokan-earth/60 uppercase tracking-wide">
                    Password
                  </label>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full border border-kokan-sand rounded-xl pl-10 pr-11 py-3 text-sm
                      text-kokan-earth bg-white focus:outline-none focus:ring-2
                      focus:ring-kokan-green/30 placeholder:text-kokan-earth/30"
                  />
                  <button
              ...
              <button
                ...
                className="w-full bg-kokan-green text-white py-3 rounded-xl font-bold text-sm
                  hover:bg-kokan-green/90 transition-all disabled:opacity-60 flex items-center
                  justify-center gap-2 shadow-sm shadow-kokan-green/20 mt-1"
              >
                {loading
              ...
              {otpStep === "email" && (
                <form onSubmit={handleSendOTP} className="space-y-4" noValidate>
                  ...
                  <button
                    ...
                    className="w-full bg-kokan-green text-white py-3 rounded-xl font-bold text-sm
                      hover:bg-kokan-green/90 disabled:opacity-60 flex items-center
                      justify-center gap-2 shadow-sm shadow-kokan-green/20"
                  >
                    {loading
                      ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ...
              {otpStep === "verify" && (
                <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-2 bg-kokan-green/5 border border-kokan-green/20 rounded-xl px-4 py-3">
                    ...
                    <span className="text-sm text-kokan-earth font-medium truncate flex-1">{email}</span>
                    <button
                      onClick={() => { setOtpStep("email"); setOtp(""); setError("") }}
                      className="text-xs text-kokan-green hover:underline flex-shrink-0"
                    >
                      Change
                    </button>
                  </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border-2 border-kokan-sand
              bg-white text-kokan-earth py-3 rounded-xl font-semibold text-sm
              hover:bg-kokan-sand/10 hover:border-kokan-earth/30 transition-all disabled:opacity-60"
          >
            {googleLoading ? (
          ...
          <Link
            href="/"
            className="flex items-center justify-center gap-2 w-full border border-kokan-sand/60
              rounded-xl py-3 text-sm text-kokan-earth/50 hover:bg-kokan-sand/10 transition-colors mt-3"
          >
