  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (code === "auth/user-not-found" || code === "auth/invalid-email") {
        // Don't reveal if email exists — still show success (security best practice)
        setState("success");
      } else {
              ...
              {[
                ...
                <div key={item.step} className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded-full bg-kokan-sand/30 border border-kokan-sand/50 flex items-center justify-center flex-shrink-0">
                    <span className="text-kokan-sand text-xs font-bold">
                      {item.step}
                    </span>
                  </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="lg:hidden text-center mb-8">
            <Link href="/" className="inline-flex items-center gap-2">
              <span className="text-2xl">🌿</span>
              <span className="font-playfair text-xl font-bold text-kokan-earth">
                Visit Kokan
              </span>
            </Link>
          </div>
          ...
          {state === "success" ? (
            ...
            <div className="text-center space-y-5">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700 text-left leading-relaxed">
                📬 Don't see it? Check your spam or junk folder. The link
                expires in 1 hour.
              </div>
